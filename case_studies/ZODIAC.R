...
for (i in 1:nrow(d0)) {
  ...
  COMBINED_HR_values <- try(COMBINED_HR(t=t_values,
                                    rho0=d0$rho[i],
  ...
  ##-- Store information
  if(class(COMBINED_HR_values)[1]!='try-error') d1[i,] <- as.numeric(unlist(COMBINED_HR_values)) 
...
############################################################
##-- Events gAHR and AHR
d$e_gAHR <- schoendfeld.formula(alpha=alpha,power=power,HR = d$gAHR_tau_100)
d$e_AHR <- schoendfeld.formula(alpha=alpha,power=power,HR = d$AHR_tau_100)
...
for(j in ite.ini:n.SETTINGS){
  ...
  power_gAHR[j] <- sum(res_gAHR<alpha,na.rm=TRUE)/nsim
...
##-- Save image
save(dd,file=paste0('../outputs/ZODIAC_3.RData'))  # After changing the code for cause-specific functions
