# calculate scale parameter in control group for case 3 and frank copula
lambda_weibull <- function(b,beta,lambda_target,tau=1){
  lambda <- dweibull(tau,beta,b)/(1-pweibull(tau,beta,b))
  return(lambda-lambda_target)
}

# Would it be possible to joint the next two functions (case2 and case 3?)
scale_parameter_case_2 <- function(b11,beta1,beta2,b10,b20,b21,copula='Frank',theta=2,HR_target=0.8,tau=1){
  
  ##-- Survival for both endpoints
  ST10 <- exp(-(tau/b10)^beta1)
  ST11 <- exp(-(tau/b11)^beta1)
  ST20 <- exp(-(tau/b20)^beta2)
  ST21 <- exp(-(tau/b21)^beta2)
  ...
  S_10 <- 1-pweibull(tau,beta1,b10) 
  S_20 <- 1-pweibull(tau,beta2,b20) 
  S_11 <- 1-pweibull(tau,beta1,b11) 
  S_21 <- 1-pweibull(tau,beta2,b21)
  
  ##-- Frank
  ##-- Endpoint 1 control group
  lambda_CS_10_1 <- 1/(Sstar0 * (exp(-theta) - 1))
  lambda_CS_10_2 <- exp(-theta*S_10) * (exp(-theta*S_20) - 1)/(exp(-theta*Sstar0))
  lambda_CS_10_3 <- dweibull(tau,beta1,b10)
  lambda_CS_10 <-  lambda_CS_10_1 * lambda_CS_10_2 * lambda_CS_10_3
  
  ##-- Endpoint 1 Treated group
  lambda_CS_11_1 <- 1/(Sstar1 * (exp(-theta) - 1))
  lambda_CS_11_2 <- exp(-theta*S_11) * (exp(-theta*S_21) - 1)/(exp(-theta*Sstar1))
  lambda_CS_11_3 <- dweibull(tau,beta1,b11)
  lambda_CS_11 <-  lambda_CS_11_1 * lambda_CS_11_2 * lambda_CS_11_3
  
  return(HR_target-lambda_CS_11/lambda_CS_10)
}

scale_parameter_case_3 <- function(b21,beta1,beta2,b10,b20,b11,copula='Frank',theta=2,HR_target=0.8,tau=1){
  ...
  ##-- Frank
  ##-- Endpoint 2 control group
  lambda_CS_20_1 <- 1/(Sstar0 * (exp(-theta) - 1))
  lambda_CS_20_2 <- exp(-theta*S_20) * (exp(-theta*S_10) - 1)/(exp(-theta*Sstar0))
  lambda_CS_20_3 <- dweibull(tau,beta2,b20)
  lambda_CS_20 <-  lambda_CS_20_1 * lambda_CS_20_2 * lambda_CS_20_3
  
  ##-- Endpoint 2 Treated group
  lambda_CS_21_1 <- 1/(Sstar1 * (exp(-theta) - 1))
  lambda_CS_21_2 <- exp(-theta*S_21) * (exp(-theta*S_11) - 1)/(exp(-theta*Sstar1))
  lambda_CS_21_3 <- dweibull(tau,beta2,b21)
  lambda_CS_21 <-  lambda_CS_21_1 * lambda_CS_21_2 * lambda_CS_21_3
  
  
  return(HR_target-lambda_CS_21/lambda_CS_20)
}

scale_parameter_case_4 <- function(b,beta1,beta2,b10,b20,copula='Frank',
                                   theta=2,HR1_target=0.8,HR2_target=0.8,tau=1){
  ...
  return(c(HR1=HR1_target-lambda_CS_11/lambda_CS_10,
           HR2=HR2_target-lambda_CS_21/lambda_CS_20))  
}
...
MarginalsSelection <- function(beta1,beta2,HR1,HR2,p1,p2,case,theta,copula='Frank') 
{
  ...
  ## -- Case 3 -----------------------
    ...
    ##-- Find p1_tilda and p2_tilda (can be remove?)
    p2_tilda <- p2
    p2_aux <- pweibull(1,beta2,b20)
    # print(p2_aux)
    p1_tilda <- integrate(function(u) {
      sapply(u,function(u) { 
        integrate(function(v){dcopula(u,v,theta)},lower=1-exp((b10*(-log(1-u))^(1/beta1))^beta2*log(1-p2_aux)),upper=1)$value
      })
    },lower=0 , upper=1-exp(-1/b10^beta1))$value
  ...
  # Scale parameters for group 1 b11,b21 (Although we do not need to compute the scale parameters for group 1 (b11, b21) to calculate the ARE)
  lambda_10 <- dweibull(1,beta1,b10)/(1-pweibull(1,beta1,b10)) # Marginal hazard control group endpoint 1
  lambda_20 <- dweibull(1,beta2,b20)/(1-pweibull(1,beta2,b20)) # Marginal hazard treated group endpoint 2
  
  ##-- Case 2
  if(case==2){   # Frank copula
    lambda_21 <- HR2*lambda_20
    b21 <- uniroot(f = lambda_weibull,lower=0.1,upper=10^5,
                   beta=beta2,lambda_target=lambda_21,tau=1,extendInt="yes")$root
    
    b11 <- uniroot(f = scale_parameter_case_2,lower=0.5,upper=10^5,
                   beta1=beta1,beta2=beta2,b10=b10,b20=b20,b21=b21,
                   copula='Frank',theta=theta,HR_target=HR1,tau=1,extendInt="yes")$root
  }
  

  
  ##-- Case 3
  if(case==3){   # Frank copula
    lambda_11 <- HR1*lambda_10
    b11 <- uniroot(f = lambda_weibull,lower=0.1,upper=10^5,
                   beta=beta1,lambda_target=lambda_11,tau=1,extendInt="yes")$root
    b21 <- uniroot(f = scale_parameter_case_3,lower=0.5,upper=10^5,
                   beta1=beta1,beta2=beta2,b10=b10,b20=b20,b11=b11,
                   copula='Frank',theta=theta,HR_target=HR2,tau=1,extendInt="yes")$root
  }

  ##-- Case 4
  if(case==4){   # Frank copula
    res <- multiroot(f = scale_parameter_case_4,start=c(1,1),
                     beta1=beta1,beta2=beta2,b10=b10,b20=b20,
                     copula='Frank',theta=theta,
                     HR1_target=HR1,HR2_target=HR2,tau=1)$root
    b11 <- res[1]
    b21 <- res[2]
  }
  
  # lambda_CS_10 <- p1_tilda*lambda_10
  # lambda_CS_20 <- p2_tilda*lambda_20
  # lambda_CS_11 <- lambda_CS_10*HR1
  # lambda_CS_21 <- lambda_CS_20*HR2

  # res <- multiroot(f=a,start=c(0.5,0.5),positive=TRUE,
  #                  beta1=beta1,beta2=beta2,
  #                  dcopula=dcopula,theta=theta,
  #                  hazards=c(lambda_CS_11,lambda_CS_21))
  
  # b11 <- res$root[1]+0.00001
  # b21 <- res$root[2]+0.00001
